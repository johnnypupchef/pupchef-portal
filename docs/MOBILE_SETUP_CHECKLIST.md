...
2. **Build + sync** — After any web change you want in the app: `npm run mobile:build` (runs `vite build` + `cap sync` for **both** platforms). If you don’t have full **Xcode + CocoaPods** yet (e.g. Linux/CI), use `npm run mobile:build:android` to build and sync **Android only**; run full `mobile:build` on a Mac with Xcode for iOS `pod install`.
